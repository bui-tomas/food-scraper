...
class FoodScraper():
    ...
    async def get_product_urls(self, page, base_url):
        '''
        ...
        pagination_buttons = await page.locator('button[aria-label^="Stránka"]').all()
        ...
        for page_num in range(1, total_pages + 1):
            ...
            await page.wait_for_selector('img[alt^="Obrázok produktu"]', timeout=10000)
            products = await page.locator('div[style*="flex-basis: calc(33.3333%"]').all()
            
            # Extract href from each product
            for product in products:
                link_element = page.locator('a strong').first
    ...
    async def extract_product_data(self, page, product_url):
        ...
        for idx, button in enumerate(retailer_buttons):
            try:
                ...
                retailer_data['retailer'] = await retailer_img.get_attribute('alt')
                
                price_strong = button.locator('p strong').first
                retailer_data['price_with_vat'] = (await price_strong.text_content()).strip()

                price_without_vat = button.locator('p.sc-dntSTA.djIdwR.sc-cAnIvK.jUAbQe').first
                retailer_data['price_without_vat'] = (await price_without_vat.text_content()).strip()
                
                unit_price = button.locator('p.sc-dntSTA.djIdwR.sc-fYRIQK.hhKUxF').first
                retailer_data['unit_price'] = (await unit_price.text_content()).strip()

                discount = button.locator('div[aria-labelledby="header_discount"] p').first
                retailer_data['discount_end_date'] = (await discount.text_content()).strip()
                ...
                if idx == 0:
                    product_name_element = panel.locator('h3.sc-kcLKEh.llLrQu').first
                    main_product_name = (await product_name_element.text_content()).strip()
                ...
                for dt_elem in dt_elements:
                    try:
                        label = (await dt_elem.locator('strong').first.text_content(timeout=2000)).strip()
    ...
    async def scrape_single_product(self, browser, url, semaphore):
        '''
        ...
        async with semaphore:
            ...
            try:
                ...
                await page.wait_for_selector('button.sc-dTvVRJ.jLGMQZ', state='attached', timeout=10000)
                product_data = await self.extract_product_data(page, url)
                return product_data
            except Exception as e:
                print(f'Error scraping {url[0]}: {e}')
                return []
            finally:
                await page.close()
    ...
    async def scrape_page(self):
        ...
        async with async_playwright() as p:
            ...
            # Phase 1: URL Discovery (sequential)
            for cat in self.cats:
                ...
                urls = await self.get_product_urls(page, cat)
                if urls:
            ...
            # Phase 2: Parallel Product Scraping
            print('📦 Scraping product data...')
            semaphore = asyncio.Semaphore(3)  # Max 10 concurrent pages
            
            tasks = [self.scrape_single_product(browser, url, semaphore) for url in all_urls]
            
            all_products = []
            for coro in tqdm.as_completed(tasks, total=len(all_urls), desc='Scraping', unit='product'):
